...
class WC_Store_pickup_Admin {
	...
	public function display_store_meta_box( $store_info ) {
	    // Retrieve current data based on store ID
	    ...
	    $store_hours	= esc_html( get_post_meta( $store_info->ID, 'store_hours', true ) ); echo $store_email.' - '.$store_hours.' final';
	    ?>
	    <table>
	        <tr>
	        ...
	        <tr>
	            <td style="width: 30%"><?php _e('Hours','woocommerce_store_pickup'); ?></td>
	            <td><textarea rows=4 name="store_pickup_hours" style="margin: 1px; width:100%;" placeholder="9:00 - 12:00 (Mon - Sat)
8:00 - 16:00 (Sun)"><?php echo $store_hours; ?></textarea></td>
	        </tr>
	    </table>
	    <?php
	}
	public function save_store_pickup_fields( $store_pickup_id, $store_pickup ) {
	    // Check post type for pickup store
	    if ( $store_pickup->post_type == 'pickup_stores' ) {
	        ...
	        if ( isset( $_POST['store_pickup_hours'] ) && $_POST['store_pickup_hours'] != '' ) {
	            update_post_meta( $store_pickup_id, 'store_hours', $_POST['store_pickup_hours'] );
	        }
	    }
	}
	...
	public function product_add_store_pickup_checkbox( $options ) { 
		$pk = get_post_meta( get_the_ID(), 'is_pickup'); 
		if( $pk[0] )
			$pk = 'yes';
		else
			$pk = 'no';
	...
	public function product_save_store_pickup_meta( $post_id ) {
		...
		if( isset( $_POST['_wc_booking_store_pickup'] ))
